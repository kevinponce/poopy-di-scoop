      ...
      if (!htmlCheck && !this.skipEach && each) {
        ...
        */
        ...
        if (!skipRender) {
          ...
          for (var i = 0; i < numberOfAttrs; i++) {
            ...
            if (htmlCheck || attr.key !== 'each') {
              if (!htmlCheck && typeof attr.value === 'string') {
                ...
                if (attr.key === 'class' && !addedAddNamespacedClass && this.addNamesapce && namespace && fmt !== _const.TEST) {
          ...
          if (!addedAddNamespacedClass && this.addNamesapce && namespace && fmt !== _const.TEST) {
