...
class HTMLElement extends Node {
     ...
     */
    constructor(name, keyAttrs, rawAttrs, index) {
        super();
        this.classNames = [];
        /**
         * Node Type declaration.
...
const kAttributePattern = /(^|\s)(id|class|required|raw)\s*=\s*("([^"]+)"|'([^']+)'|(\S+))/ig;
...
function parse(data, options) {
    ...
    while (match = kMarkupPattern.exec(data)) {
        ...
        if (!match[1]) {
            ...
            raw = raw || (attrs['raw'] === 'text')
            if (attrs['raw'] === 'text') {
                var closeMarkup = '</' + match[2] + '>';
                var index = data.indexOf(closeMarkup, kMarkupPattern.lastIndex);
                let text = data.substring(kMarkupPattern.lastIndex, index)

                currentParent = currentParent.appendChild(new HTMLElement(match[2], attrs, match[3], match['index']));
